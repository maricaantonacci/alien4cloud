...
public class ConstraintPropertyService {
  ...
  public void checkPropertyConstraint(String propertyName, Object propertyValue,
      PropertyDefinition propertyDefinition, ArchiveRoot archive)
    ...
    if (propertyValue instanceof String) {
      ...
      checkComplexPropertyConstraint(propertyName, (Map<String, Object>) propertyValue,
          propertyDefinition, archive);
    } else if (propertyValue instanceof List) {
      checkListPropertyConstraint(propertyName, (List<Object>) propertyValue, propertyDefinition,
          archive);
      // TODO Customization Reply
      ...
      checkListPropertyConstraint(propertyName, ((ListPropertyValue) propertyValue).getValue(),
          propertyDefinition, archive);
    } else if (propertyValue instanceof ComplexPropertyValue) {
      checkComplexPropertyConstraint(propertyName,
          ((ComplexPropertyValue) propertyValue).getValue(), propertyDefinition, archive);
    } else if (propertyValue instanceof FunctionPropertyValue) {
      // TODO VALIDATE
      // END
    } else {
      throw new InvalidArgumentException(
          "Not expecting to receive constraint validation for other types than String, Map or List as "
   ...
   *
   * @param propertyName
   *          Property name (mainly used to create a comprehensive error message)
   * @param stringValue
   *          Tested property value
   * @param propertyDefinition
   *          Full property definition with type, constraints, default value,...
   * @throws ConstraintViolationException
   * @throws ConstraintValueDoNotMatchPropertyTypeException
   */
  // FIXME check type first, and constraint after
  public void checkSimplePropertyConstraint(final String propertyName, final String stringValue,
    ...
    if (propertyDefinition.getConstraints() != null
      ...
      for (PropertyConstraint constraint : propertyDefinition.getConstraints()) {
        ...
        try {
          ...
          // ConstraintValueDoNotMatchPropertyTypeException is not supposed to be raised here (only
          // in constraint definition validation)
          log.info("Constraint introspection error for property <" + propertyName + "> value <"
              + stringValue + ">", e);
          throw new ConstraintTechnicalException("Constraint introspection error for property <"
              + propertyName + "> value <" + stringValue + ">", e);
        }
      }
    } else {
      ...
      try {
        ...
        consInformation = new ConstraintInformation(propertyName, null, stringValue,
            propertyDefinition.getType());
        throw new ConstraintValueDoNotMatchPropertyTypeException(e.getMessage(), e,
            consInformation);
      }
    }
  }

  public void checkDataTypePropertyConstraint(String propertyName,
      Map<String, Object> complexPropertyValue, PropertyDefinition propertyDefinition,
      ArchiveRoot archive)
      throws ConstraintViolationException, ConstraintValueDoNotMatchPropertyTypeException {
    IndexedDataType dataType = ToscaParsingUtil
        .getDataTypeFromArchiveOrDependencies(propertyDefinition.getType(), archive, searchService);
    if (dataType == null) {
    ...
    for (Map.Entry<String, Object> complexPropertyValueEntry : complexPropertyValue.entrySet()) {
      if (dataType.getProperties() == null
          || !dataType.getProperties().containsKey(complexPropertyValueEntry.getKey())) {
        throw new ConstraintViolationException("Complex type " + propertyDefinition.getType()
            + " do not have nested property with name " + complexPropertyValueEntry.getKey()
            + " for property " + propertyName);
      } else {
        Object nestedPropertyValue = complexPropertyValueEntry.getValue();
        PropertyDefinition nestedPropertyDefinition =
            dataType.getProperties().get(complexPropertyValueEntry.getKey());
        checkPropertyConstraint(propertyName + "." + complexPropertyValueEntry.getKey(),
            nestedPropertyValue, nestedPropertyDefinition, archive);
      }
    }
  }

  public void checkListPropertyConstraint(String propertyName, List<Object> listPropertyValue,
      PropertyDefinition propertyDefinition, ArchiveRoot archive)
      throws ConstraintValueDoNotMatchPropertyTypeException, ConstraintViolationException {
    PropertyDefinition entrySchema = propertyDefinition.getEntrySchema();
    for (int i = 0; i < listPropertyValue.size(); i++) {
      checkPropertyConstraint(propertyName + "[" + String.valueOf(i) + "]",
          listPropertyValue.get(i), entrySchema, archive);
    }
  }

  public void checkMapPropertyConstraint(String propertyName,
      Map<String, Object> complexPropertyValue, PropertyDefinition propertyDefinition,
      ArchiveRoot archive)
      throws ConstraintValueDoNotMatchPropertyTypeException, ConstraintViolationException {
    PropertyDefinition entrySchema = propertyDefinition.getEntrySchema();
    for (Map.Entry<String, Object> complexPropertyValueEntry : complexPropertyValue.entrySet()) {
      checkPropertyConstraint(propertyName + "." + complexPropertyValueEntry.getKey(),
          complexPropertyValueEntry.getValue(), entrySchema, archive);
    }
  }
   ...
   *          the definition
   * @param archive
   *          the archive containing the definition
   * @throws ConstraintViolationException
   * @throws ConstraintValueDoNotMatchPropertyTypeException
   */
  public void checkComplexPropertyConstraint(String propertyName,
      Map<String, Object> complexPropertyValue, PropertyDefinition propertyDefinition,
      ArchiveRoot archive)
      throws ConstraintViolationException, ConstraintValueDoNotMatchPropertyTypeException {
   ...
   * @throws Exception
   */
  private void checkBasicType(final String propertyName,
      final PropertyDefinition propertyDefinition, final String propertyValue) {
    ...
    try {
      ...
      throw new NumberFormatException(
          propertyName + "'s value is not a number [ " + propertyValue + " ]");
    } catch (InvalidVersionException e) {
      throw new InvalidVersionException(
          propertyName + "'s value is not a version [ " + propertyValue + " ]");
    }
  }
}
