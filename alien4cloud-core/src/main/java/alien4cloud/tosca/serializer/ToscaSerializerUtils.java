...
/**
 * Tools for serializing in YAML/TOSCA. ALl methods should be static but did not found how to use
 * statics from velocity.
 */
public class ToscaSerializerUtils {
  ...
  /**
   * Render the scalar: when it contain '[' or ']' or '{' or '}' or ':' or '-' or ',', then quote
   * the scalar.
   */
  ...
  /**
   * Render a description. If the string contain CRLF, then render a multiline literal preserving
   * indentation.
   */
  ...
  /**
   * Check if the map is not null, not empty and contains at least one not null value. This function
   * is recursive:
   * <ul>
  ...
  /**
   * Check if the list is not null, not empty and contains at least one not null value. This
  ...
  public String renderFunctionAndConcat(AbstractPropertyValue apv) {
    ...
    builder.append(functionName).append(": [ ").append(getCsvToString(parameters, true))
        .append(" ]");
    return builder.toString();
  }
