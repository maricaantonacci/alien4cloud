...
public class RelationshipTemplatesParser extends DefaultDeferredParser<Map<String, RelationshipTemplate>> {
    ...
    public Map<String, RelationshipTemplate> parse(Node node, ParsingContextExecution context) {
        ...
        for (Node child : children) {
            ...
            if (valueNode instanceof ScalarNode) {
                ...
                buildAndAddRelationhip(valueNode, nodeTemplate, toscaRequirementName, toscaRequirementTargetNodeTemplateName, null, null, context, result, null, null);
                continue;
                ...
                for (NodeTuple mappingValueNodeChilds : mappingValueNode.getValue()) {
                    if (mappingValueNodeChilds.getKeyNode() instanceof ScalarNode) {
                        String keyNodeName = ((ScalarNode) mappingValueNodeChilds.getKeyNode()).getValue();
                        if (keyNodeName != null) {
                            switch (keyNodeName) {
                                case "properties":
                                    if (mappingValueNodeChilds.getValueNode() instanceof MappingNode) {
                                    ...
                                    break;
                                case "node":
                                    if (mappingValueNodeChilds.getValueNode() instanceof ScalarNode) {
                                        toscaRequirementTargetNodeTemplateName = ((ScalarNode) mappingValueNodeChilds.getValueNode()).getValue();
                                    }
                                    break;
                                case "capability":
                                    if (mappingValueNodeChilds.getValueNode() instanceof ScalarNode) {
                                        tosca_capability = ((ScalarNode) mappingValueNodeChilds.getValueNode()).getValue();
                                    }
                                    break;
                                case "relationship":
                                    if (mappingValueNodeChilds.getValueNode() instanceof ScalarNode) {
                                        relationshipType = ((ScalarNode) mappingValueNodeChilds.getValueNode()).getValue();
                                    } else if (mappingValueNodeChilds.getValueNode() instanceof MappingNode) {
                                        MappingNode relationshipNode = ((MappingNode) mappingValueNodeChilds.getValueNode());
                ...
                buildAndAddRelationhip(valueNode, nodeTemplate, toscaRequirementName, toscaRequirementTargetNodeTemplateName, tosca_capability,
                        relationshipType, context, result, relationshipProperties, relationshipInterfaces);
            }
    ...
    private void buildAndAddRelationhip(Node node, NodeTemplate nodeTemplate, String toscaRequirementName, String toscaRequirementTargetNodeTemplateName,
        ...
        RelationshipTemplate relationshipTemplate = buildRelationshipTemplate(node, nodeTemplate, toscaRequirementName, toscaRequirementTargetNodeTemplateName,
                capabilityType, relationshipType, context, relationshipProperties, relationshipInterfaces);
    ...
    private RelationshipTemplate buildRelationshipTemplate(Node node, NodeTemplate nodeTemplate, String toscaRequirementName,
        ...
        Map<String, Interface> interfaces = IndexedModelUtils.mergeInterfaces(indexedRelationshipType.getInterfaces(), relationshipInterfaces, true);
